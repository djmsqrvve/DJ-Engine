...
validate:
	...
	if [ "$$test_count" -lt 510 ]; then \
		echo "FAILED: Test count regression ($$test_count < 510)"; exit 1; \
	else \
		echo "Test count: $$test_count (>= 510 minimum)"; \
	fi
	@echo ""
...
qa: validate
	...
	@echo "[1/5] RPG Demo..."; timeout -s KILL 5s cargo run -p rpg_demo > /tmp/dj_qa_rpg.log 2>&1 && echo "  rpg_demo: PASS" || echo "  rpg_demo: KILLED (timeout or crash, see /tmp/dj_qa_rpg.log)"
	@echo "[2/5] DoomExe..."; timeout -s KILL 5s cargo run -p doomexe --bin doomexe > /tmp/dj_qa_doom.log 2>&1 && echo "  doomexe: PASS" || echo "  doomexe: KILLED (timeout or crash, see /tmp/dj_qa_doom.log)"
	@echo "[3/5] Helix RPG..."; timeout -s KILL 5s cargo run -p helix_rpg > /tmp/dj_qa_helix.log 2>&1 && echo "  helix_rpg: PASS" || echo "  helix_rpg: KILLED (timeout or crash, see /tmp/dj_qa_helix.log)"
	@echo "[4/5] Stratego..."; timeout -s KILL 5s cargo run -p stratego --bin stratego > /tmp/dj_qa_stratego.log 2>&1 && echo "  stratego: PASS" || echo "  stratego: KILLED (timeout or crash, see /tmp/dj_qa_stratego.log)"
	@echo "[5/5] Iso Sandbox..."; timeout -s KILL 5s cargo run -p iso_sandbox > /tmp/dj_qa_iso.log 2>&1 && echo "  iso_sandbox: PASS" || echo "  iso_sandbox: KILLED (timeout or crash, see /tmp/dj_qa_iso.log)"
	@echo ""
