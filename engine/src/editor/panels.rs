...
use crate::data::{
    ...
    update_loaded_custom_document_typed, CustomDocumentRegistry, DocumentLink, DocumentLinkTarget,
    DocumentRef, EditorDocumentRoute, LoadedCustomDocuments, PreviewProfilePayload, Project,
...
pub(super) fn validation_issue_color(severity: ValidationSeverity) -> Color32 {
    match severity {
...
pub(super) fn field_path_matches(issue: &ValidationIssue, field_path: &str) -> bool {
...
pub(super) fn first_document_field_issue<'a>(
    loaded_documents: &'a LoadedCustomDocuments,
    selected: &DocumentRef,
    field_path: &str,
...
fn draw_document_inspector(
    ...
    world.resource_scope::<LoadedCustomDocuments, _>(|_, mut loaded_documents| {
        ...
        // Payload property editor: structured editing for nested fields.
        if let Some(parsed) = &document.document {
            if let Some(obj) = parsed.payload.as_object() {
                ui.separator();
                ui.label(RichText::new("Payload Properties").strong());
                let mut pending_edits = Vec::new();
                let mut keys: Vec<&String> = obj.keys().collect();
                keys.sort();
                for key in keys {
                    if let Some(value) = obj.get(key) {
                        let field_path = format!("payload.{key}");
                        super::property_widgets::draw_value_editor(
                            ui,
                            &loaded_documents,
                            selected_ref,
                            &field_path,
                ...
                if let Some(project) = project {
                    for edit in pending_edits {
                        // Strip the "payload." prefix for the nested update helper.
                        let nested_path = edit
                            .field_path
                            .strip_prefix("payload.")
                            .unwrap_or(&edit.field_path);
                        let _ = crate::data::update_loaded_custom_document_nested_value(
                            &mut loaded_documents,
                            project,
                            registry,
                            &selected_ref.kind,
