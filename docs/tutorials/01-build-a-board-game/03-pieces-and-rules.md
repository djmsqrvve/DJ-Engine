# Chapter 3: Rules and Combat

Define movement validation, combat resolution, and move execution.

## What You'll Add

- `rules.rs` -- Combat, movement validation, move execution (346 lines, 11 tests)

## rules.rs

> **File: `games/dev/stratego/src/rules.rs`**
...
- **Combat is simple**: higher strength wins, equal kills both, capturing the Flag wins the game. The Flag-as-attacker edge case ensures it always loses.
- **Movement validation** checks: piece exists, is yours, can move, target isn't lake, isn't friendly, is orthogonal, distance is 1 (or multi-cell for Scouts with clear path).
- **`is_clear_path`** is a private helper that ray-casts from source to destination, checking for obstacles. It does NOT check the destination (which may be an enemy to attack).
- **`valid_moves`** uses two strategies: Scouts ray-cast in 4 directions; normal pieces check `grid.neighbors()`.
- **`execute_move`** clones the source cell first (to capture the attacker before clearing), then applies the combat outcome. After combat, involved pieces become `revealed: true`.
...
No other changes to main.rs yet -- rules has no systems, just pure functions.
...
20 tests pass (4 pieces + 5 board + 11 rules). Still no visual output.

## Next

[Chapter 4: Rendering and State](04-rendering.md) -- Draw the board and pieces on screen.
